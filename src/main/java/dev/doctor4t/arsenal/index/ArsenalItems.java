...
public interface ArsenalItems {
    ...
    Item SCYTHE = create("scythe", new ScytheItem(ArsenalToolMaterials.SCYTHE, 5.0f, -3.0f,
        new Item.Settings().rarity(Rarity.COMMON)));
    Item ANCHORBLADE = create("anchorblade", new AnchorbladeItem(ArsenalToolMaterials.ANCHORBLADE, 5.0f, -3.0f,
        new Item.Settings().rarity(Rarity.COMMON)));
    ...
    private static void addCombatEntries(FabricItemGroupEntries entries) {
        entries.addAfter(Items.TRIDENT, SCYTHE);
        entries.addAfter(SCYTHE, ANCHORBLADE);
    }

    private static void addFunctionalEntries(FabricItemGroupEntries entries) {
        entries.addAfter(Items.GLOW_ITEM_FRAME, WEAPON_RACK);
    }
}

...
public class AnchorbladeEntity extends PersistentProjectileEntity {
            ...
            if (this.hasReeling()) {
                ...
                for (LivingEntity hitLivingEntity : this.getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(radius), LivingEntity::isAlive)) {
                    ...
                    if (!(strength <= 0.0)) {
                        ...
                        if (footDistance.y > distance.y) distance = footDistance;
    ...
    @Override public void setPitch(float pitch) { if (!this.hasDealtDamage()) super.setPitch(pitch); }
    @Override public void setYaw(float yaw) { if (!this.hasDealtDamage()) super.setYaw(yaw); }
    ...
    @Override protected boolean tryPickup(PlayerEntity player) { return this.isOwner(player); }
    ...
    public boolean hasDealtDamage() { return this.getAnchorFlag(0); }
    public void setDealtDamage(boolean v) { this.setAnchorFlag(0, v); }
    public boolean hasReeling() { return this.getAnchorFlag(1); }
    public void setReeling(boolean v) { this.setAnchorFlag(1, v); }
    public boolean isRecalled() { return this.getAnchorFlag(2); }
    public void setRecalled(boolean v) { if (v) this.setDealtDamage(true); this.setAnchorFlag(2, v); }

    private boolean getAnchorFlag(int flag) {
        if (flag < 0 || flag > 8) return false;
    ...
    private void setAnchorFlag(int flag, boolean value) {
        ...
        if (value) this.dataTracker.set(ANCHOR_FLAGS, (byte)(this.dataTracker.get(ANCHOR_FLAGS) | 1 << flag));
        else       this.dataTracker.set(ANCHOR_FLAGS, (byte)(this.dataTracker.get(ANCHOR_FLAGS) & ~(1 << flag)));
    }
}
